...
ParseResult InstantiateOp::parse(OpAsmParser &parser, OperationState &result) {
    ...
    if (parser.parseSymbolName(calleeAttr, getCalleeAttrName(result.name), result.attributes))
        return failure();
    ...
    if (inpTypes.size() != inputs.size() ||
        outTypes.size() != outputs.size()) {
            parser.emitError(location, "Call signature does not match operand count");
        }
