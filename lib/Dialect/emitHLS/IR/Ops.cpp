...
#include "dfg-mlir/Dialect/emitHLS/IR/Ops.cpp.inc"
    ...
    emitHLS::PragmaStorageImpl impl)
{
    ...
    result.addAttribute(
        ...
        emitHLS::PragmaStorageTypeAttr::get(builder.getContext(), type));
    result.addAttribute(
        getImplAttrName(result.name),
        emitHLS::PragmaStorageImplAttr::get(builder.getContext(), impl));
}
...
PragmaBindStorageOp::parse(OpAsmParser &parser, OperationState &result)
{
    ...
    emitHLS::PragmaStorageType storageType;
    if (typeStr == "fifo")
        storageType = emitHLS::PragmaStorageType::fifo;
    else if (typeStr == "ram_1p")
        storageType = emitHLS::PragmaStorageType::ram_1p;
    else if (typeStr == "ram_1wnr")
        storageType = emitHLS::PragmaStorageType::ram_1wnr;
    else if (typeStr == "ram_2p")
        storageType = emitHLS::PragmaStorageType::ram_2p;
    else if (typeStr == "ram_s2p")
        storageType = emitHLS::PragmaStorageType::ram_s2p;
    else if (typeStr == "ram_t2p")
        storageType = emitHLS::PragmaStorageType::ram_t2p;
    else if (typeStr == "rom_1p")
        storageType = emitHLS::PragmaStorageType::rom_1p;
    else if (typeStr == "rom_2p")
        storageType = emitHLS::PragmaStorageType::rom_2p;
    else if (typeStr == "rom_np")
        storageType = emitHLS::PragmaStorageType::rom_np;
    else
    ...
    emitHLS::PragmaStorageImpl storageImpl;
    if (implStr == "bram")
        storageImpl = emitHLS::PragmaStorageImpl::bram;
    else if (implStr == "bram_ecc")
        storageImpl = emitHLS::PragmaStorageImpl::bram_ecc;
    else if (implStr == "lutram")
        storageImpl = emitHLS::PragmaStorageImpl::lutram;
    else if (implStr == "uram")
        storageImpl = emitHLS::PragmaStorageImpl::uram;
    else if (implStr == "uram_ecc")
        storageImpl = emitHLS::PragmaStorageImpl::uram_ecc;
    else if (implStr == "srl")
        storageImpl = emitHLS::PragmaStorageImpl::srl;
    else if (implStr == "memory")
        storageImpl = emitHLS::PragmaStorageImpl::memory;
    else if (implStr == "auto")
        storageImpl = emitHLS::PragmaStorageImpl::automatic;
    else
        return parser.emitError(
                   parser.getNameLoc(),
    ...
    result.addAttribute(
        ...
        emitHLS::PragmaStorageTypeAttr::get(parser.getContext(), storageType));
    result.addAttribute(
        getImplAttrName(result.name).data(),
        emitHLS::PragmaStorageImplAttr::get(parser.getContext(), storageImpl));
...
PragmaInterfaceOp::parse(OpAsmParser &parser, OperationState &result)
{
    ...
    if (succeeded(parser.parseOptionalKeyword("offset"))) {
        ...
        emitHLS::PragmaInterfaceMasterAxiOffset axiOffset;
        if (offsetStr == "off")
            axiOffset = emitHLS::PragmaInterfaceMasterAxiOffset::off;
        else if (offsetStr == "direct")
            axiOffset = emitHLS::PragmaInterfaceMasterAxiOffset::direct;
        else if (offsetStr == "slave")
            axiOffset = emitHLS::PragmaInterfaceMasterAxiOffset::slave;
        else
...
void PragmaPipelineOp::build(
    ...
    OperationState &result,
    int64_t interval,
    emitHLS::PragmaPipelineStyle style)
{
    result.addAttribute(
    ...
    result.addAttribute(
        ...
        emitHLS::PragmaPipelineStyleAttr::get(builder.getContext(), style));
}
...
ParseResult PragmaPipelineOp::parse(OpAsmParser &parser, OperationState &result)
{
    ...
    emitHLS::PragmaPipelineStyle style;
    if (styleStr == "flp")
        style = emitHLS::PragmaPipelineStyle::flp;
    else if (styleStr == "stp")
        style = emitHLS::PragmaPipelineStyle::stp;
    else if (styleStr == "frp")
        style = emitHLS::PragmaPipelineStyle::frp;
    else
    ...
    result.addAttribute(
        "style",
        emitHLS::PragmaPipelineStyleAttr::get(parser.getContext(), style));
