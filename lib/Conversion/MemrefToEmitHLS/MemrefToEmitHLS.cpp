/// Implementation of MemrefToEmitHLS pass.
///
/// @file
...
namespace mlir {
#define GEN_PASS_DEF_CONVERTMEMREFTOEMITHLS
...
void mlir::populateMemrefToEmitHLSConversionPatterns(
    TypeConverter typeConverter,
...
namespace {
struct ConvertMemrefToEmitHLSPass
        : public impl::ConvertMemrefToEmitHLSBase<ConvertMemrefToEmitHLSPass> {
    void runOnOperation() final;
};
} // namespace

void ConvertMemrefToEmitHLSPass::runOnOperation()
{
    TypeConverter converter;
    converter.addConversion([](Type type) { return type; });
    converter.addConversion([](MemRefType memrefTy) {
        return emitHLS::ArrayType::get(
            memrefTy.getShape(),
            memrefTy.getElementType());
    });
    ...
    populateMemrefToEmitHLSConversionPatterns(converter, patterns);
...
std::unique_ptr<Pass> mlir::createConvertMemrefToEmitHLSPass()
{
    return std::make_unique<ConvertMemrefToEmitHLSPass>();
}
