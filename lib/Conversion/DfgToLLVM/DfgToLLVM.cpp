...
struct LoopOpLowering : public OpConversionPattern<LoopOp> {
        ...
        ConversionPatternRewriter &rewriter) const override
    {
        ...
        for (auto inItem : op.getInChans()) regionArgs.erase(inItem);
        for (auto outItem : op.getOutChans()) regionArgs.erase(outItem);
        ...
        assert(parentRegion.getBlocks().size() == 2);
