...
struct OlympusPushNLowering : public OpConversionPattern<PushNOp> {
...
/// with the Olympus memory layout.
struct OlympusPullNLowering : public OpConversionPattern<PullNOp> {
...
ProcessOp insertOlympusWrapperOp(InstantiateOp instantiation)
{
    ...
    SmallVector<Value> alveoInputs(
        ioChans.begin(),
        ioChans.begin() + 1 + instantiation.getInputs().size());

    //   call host
    FlatSymbolRefAttr alveoHostFunc = getOrInsertFunc(
        rewriter,
        module,
        alveoHostCall,
        std::nullopt,
...
{
    patterns.add<OlympusPushNLowering>(typeConverter, patterns.getContext());

    patterns.add<OlympusPullNLowering>(typeConverter, patterns.getContext());
}
...
void ConvertDfgInsertOlympusWrapperPass::runOnOperation()
{
    ...
    target.addDynamicallyLegalOp<PushNOp>([&](PushNOp op) {
        // NOTE(feliix42): This might fail for items with parent != OperatorOp.
        // Is that legal?
        OperatorOp parent = op->getParentOfType<OperatorOp>();
        bool legal = true;
        for (OperatorOp newOp : newOps) legal = legal && (newOp != parent);

        return legal;
    });
    target.addDynamicallyLegalOp<PullNOp>([&](PullNOp op) {
        // NOTE(feliix42): This might fail for items with parent != OperatorOp.
        // Is that legal?
        OperatorOp parent = op->getParentOfType<OperatorOp>();
        bool legal = true;
